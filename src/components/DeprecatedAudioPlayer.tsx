...
function DeprecatedAudioPlayer({ audioID, setAudioID, songs }: AudioPlayerProps) {
  ...
  const audioStatus = async (status: AVPlaybackStatus) => {
    ...
    if (status.didJustFinish) {
      ...
      if (autoplay && shuffle) {
        ...
        if (songsArr.length === 0) songsArr = [...songs];

        setSongsLeft(songsArr);
        setSong(songsArr[useRandomInt(0, songsArr.length - 1)]);
      } else if (autoplay) setSong(songs[(songs.indexOf(song!) + 1) % songs.length]);
      else await endPlayback();
    }
  };
  ...
  const moveTo = async (position: number) => {
    ...
    if (status.isLoaded && status.isPlaying) await AudioPlayer.current.setPositionAsync(position * 1000);
  };
  ...
  const pause = async () => {
    if (isPaused) await AudioPlayer.current.playAsync();
    else await AudioPlayer.current.pauseAsync();
  ...
  const playResource = async () => {
    ...
    if (isLoaded) await AudioPlayer.current.unloadAsync();
    ...
    if (status.isLoaded) {
      ...
      resetProgress();
      if (!status.isPlaying) await AudioPlayer.current.playAsync();
    }
  };
  ...
  const toggleAutoplay = () => {
    if (autoplay) setShuffle(false);
    setAutoplay(value => !value);
  };

  const toggleShuffle = () => {
    if (!shuffle) setAutoplay(true);
...
export default DeprecatedAudioPlayer;
