...
export default function useCompare<T>(
  arr: T[],
  extractor: (item: T) => Date | number | string,
  reverse?: boolean,
): T[] {
  ...
  if (!reverse) return arr.sort((a, b) => compareFun(extractor(a), extractor(b)));
  else return arr.sort((a, b) => compareFun(extractor(a), extractor(b))).reverse();
}
