...
function SongsAdder({ close, playListID, refreshPlayList, isOpen }: SongsAdderProps) {
  ...
  const getSongs = useCallback(async () => {
    setSongs(
      useCompare(
        (await SongsController.store.findAsync({
          $not: { 'musicly.playListsIDs': playListID },
        })) as SavedSongMetadata[],
        item => item.musicly.file.downloadDate,
        true,
      ),
    );
  }, []);
    ...
    setSearchedSongs([
      ...songs.filter(
        song =>
          ...
          song.channel.name.toLowerCase().match(searchQuery.toLowerCase()),
      ),
    ]);
  }, [searchQuery]);
      ...
      <Modal.Content>
        ...
        <Modal.Body>
          <Input
            ...
            borderRadius={'4'}
            fontSize={'md'}
            InputLeftElement={<Icon color="gray.400" m={2} ml={3} size="6" as={<MaterialIcons name="search" />} />}
              ...
              <TouchableOpacity key={song.id} onPress={() => addToPlaylist(song.id)}>
                <Box bg={'gray.100'} mb={1} mt={1} p={2} rounded={'md'}>
                  ...
                  <VStack>
                    ...
                    <Text fontSize={'xs'}>
                      {song.channel.name} • {song.metadata.duration.seconds}s
                    </Text>
                  </VStack>
                </Box>
              </TouchableOpacity>
