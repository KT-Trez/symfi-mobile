...
function SongsSearch() {
  ...
  const fetchSongs = useCallback(async () => {
    ...
    try {
      ...
      ToastAndroid.showWithGravity(
        `Server returned an error [${err ?? 'unknown'}]. Try again later`,
        ToastAndroid.SHORT,
        ToastAndroid.BOTTOM,
      );
    } finally {
  ...
  useEffect(() => {
    if (selectedSong) showActionSheet();
  }, [selectedSong]);
  ...
  return (
    <>
      ...
      <SongActions data={selectedSong} hide={hideActionSheet} isVisible={actionSheetShows} />
    </>
  );
}
