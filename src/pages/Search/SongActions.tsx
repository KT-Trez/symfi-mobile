...
function SongActions({ data, hide, isVisible }: SongActionsProps) {
  ...
  const checkDownloadedStatus = useCallback(async () => {
    ...
    if ((await SongsController.countAsync({ id: data.id })) > 0) setIsDownloaded(true);
  ...
  const downloadSong = useCallback(async () => {
    ...
    if (status !== MediaLibrary.PermissionStatus.GRANTED) {
      ...
      return ToastAndroid.showWithGravity(
        "Can't save a file without media library permission.",
        ToastAndroid.LONG,
        ToastAndroid.BOTTOM,
      );
    }
  ...
  return (
    <Actionsheet isOpen={isVisible} onClose={hide}>
      <Actionsheet.Content>
        ...
        <Actionsheet.Item onPress={hide} startIcon={<Icon as={MaterialCommunityIcons} name={'close'} size="6" />}>
          Cancel
        </Actionsheet.Item>
      </Actionsheet.Content>
    </Actionsheet>
